...
let FetchingService = require("../fetching");
let ServiceLoader;

let warn = function(msg, params) {
  logger.warn("[EntityService warn]: " + msg, params);
};

let info = function(msg, params) {
  logger.info("[EntityService info]: " + msg, params);
};

let debug = function(msg, params) {
  logger.debug("[EntityService debug]: " + msg, params);
};

let exception = function(msg, params) {
  throw new Error("[EntityService exception]: " + msg, params);
};
...
class EntityService {
  ...
  constructor(schema, app, db) {
    ...
    if (!ServiceLoader) ServiceLoader = require("./index.js");

    if (!schema.settings)
      exception(
        `No settings of service '${self.name}'! Please create a settings object in service schema!`
      );
    ...
    if (!self.name)
      exception(
        `No name of service '${self.name}'! Please set in settings of service schema!`
      );

    if (!self.namespace && (settings.rest || settings.ws || settings.graphql))
      exception(
        `No namespace of service '${self.name}'! Please set in settings of service schema!`
      );
    ...
    let cachingWrapper = this.newCachingWrapper();
    ...
    if (schema.actions && _.isObject(schema.actions)) {
      ...
      _.forIn(schema.actions, (action, name) => {
        ...
        if (_.isFunction(action.handler)) {
          ...
          if (action.cache) func = cachingWrapper(action, func);

          self.actions[name] = func;
        }
        self.actions[name].settings = action;
        self.actions[name].settings.name =
          self.actions[name].settings.name || name;
    ...
    if (schema.methods && _.isObject(schema.methods)) {
      _.forIn(schema.methods, (method, name) => {
        if (
          ["name", "version", "namespace", "collection", "actions"].indexOf(
            name
          ) != -1
        ) {
          warn(
            `Invalid method name '${name}' in '${self.name}' service! Skipping...`
          );
          return;
        }
        if (["toJSON", "getByID", "modelResolver"].indexOf(name) != -1) {
          warn(
            `This method name '${name}' is prohibited under 'methods' object. If you want to override the built-in method, please declare in the root of service schema! Skipping...`
          );
          return;
        }
  ...
  /**
   * Returns a new caching wrapper.
   * @returns a new cache wrapper method.
   */
  newCachingWrapper() {
    const self = this;
    return function(action, handler) {
      return function(ctx) {
        ...
        return self.getFromCache(cacheKey).then(cachedJSON => {
          if (cachedJSON != null) {
            // Found in the cache!
            return cachedJSON;
          }
  ...
  toJSON(docs, propFilter) {
    let func = function(doc, propFilter) {
      let json = doc.toJSON();
      if (propFilter != null) return _.pick(json, propFilter);
      else return json;
    };
    ...
    if (_.isString(propFilter)) propFilter = propFilter.split(" ");
  ...
  /**
	 ...
	 *
	 * Model populate looks like this :
	 * <code>
	 *  modelPopulates: {
  ...
  populateModels(docs, populateSchema) {
    ...
    if (docs != null && populateSchema) {
      let promises = [];
      // Iterate on the field modelPopulates, to watch for some aggregations to perform
      _.forIn(populateSchema, (aggServiceRef, field) => {
        /**
		 * Service name contains : a service reference and a list of field to look up
		 */
        debug("Populating models for the field", field);
        if (_.isObject(aggServiceRef) && _.isString(aggServiceRef.service)) {
          // Get Service ig "persons"
          let service = ServiceLoader.get(aggServiceRef.service);

          // Looking by references provided before
          if (service) {
            debug("Aggregate %j", field);
            debug("Aggregate Reference %j", aggServiceRef);

            if (
              (!aggServiceRef.ref || aggServiceRef.ref.length == 0) &&
              _.isFunction(service["getByID"])
            ) {
              debug("Aggregate by ID %s", field);
              // Fetch by ID is the default case
              FetchingService.fetchByID(docs, field, service, promises);
            } else if (aggServiceRef.ref && aggServiceRef.ref.length > 0) {
              debug("Aggregate by references %s", field);
              // Fetch by references
              FetchingService.fetchByReferences(
                docs,
                field,
                service,
                promises,
            ...
            throw new Error(
              "Could not find the service " + aggServiceRef.service
            );
          }
        }
      });
  ...
  /**
	 * Get model(s) using a filter. The filter is providing a key and a list of ID(s). The `ids` can be a number or an array with IDs.
	 *
	 * @param {Mixed} filter : a structure of type {key: referenceName, ids: [id1, id2, id3]}
	 * @returns {Object|Array} JSON object(s)
	 */

  findByFilter(filter) {
    debug("Find by filter is %j", filter);
    if (
    ...
    if (_.isArray(filter.ids) && filter.ids.length == 0) {
      return Promise.resolve([]);
    }

    let cacheKey = config.cacheTimeout
      ? this.getCacheKey(filter.key, filter.ids)
      : null;
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (cacheKey) {
          debug("Returns cache data from ", cacheKey);
      ...
      .then(data => {
        ...
        let query;
        if (_.isArray(filter.ids)) {
          const mongoFilter = {};
          mongoFilter[filter.key] = { $in: filter.ids };
          query = this.collection.find(mongoFilter);
        } else {
          const mongoFilter = {};
          mongoFilter[filter.key] = filter.ids;
          query = this.collection.find(mongoFilter);
        }
        ...
        return query
          .exec()
          .then(docs => {
            debug("Mongo returned data %j", docs);
          ...
          .then(json => {
            // Save to cache
            if (cacheKey) this.putToCache(cacheKey, json);
  ...
  /**
	 ...
	 */
  getByID(id) {
    if (this.collection == null || id == null) return Promise.resolve();

    if (_.isArray(id) && id.length == 0) return Promise.resolve([]);
    ...
    return Promise.resolve()
      .then(() => {
        // Try to read from cache
        if (cacheKey) return this.getFromCache(cacheKey);
        else return null;
      })
      .then(data => {
        if (data) return data;
        ...
        } else query = this.collection.findById(id);
  ...
  modelResolver(ctx, code) {
    if (this.collection == null) return Promise.resolve();
  ...
  decodeID(code) {
    ...
    throw new Error(
      `'decodeID' method not implemented in '${this.name}' service!`
    );
  }
  ...
  encodeID(id) {
    ...
    throw new Error(
      `'encodeID' method not implemented in '${this.name}' service!`
    );
  }
  ...
  notifyModelChanges(ctx, type, json) {
    ...
    ServiceLoader.emit(this.name + ":" + type, { ctx: ctx, payload: json });
